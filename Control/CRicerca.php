...
class CRicerca
{
    ...
    public static function blogHome(){
        ...
        $annunci_id =$pm::load('FAnnuncio',array(['acquistato','=','0']));
   if(!isset($annunci_id)&&!is_array($annunci_id)) $numAnnunci=0;
   elseif (isset($annunci_id)&&!is_array($annunci_id)) {
       $annunci_id = array($annunci_id);
       $numAnnunci = 1;
   }
   else $numAnnunci=count($annunci_id);
...
if($numAnnunci!=0) {
    for ($i = 0; $i < $numAnnunci; $i++) {
        ...
        if ($numAnnunci == 1) $annunci_home[] = $pm::load('FAnnuncio', array(['idAnnuncio', '=', $annunci_id[0]->getIdAnnuncio()]));
        else $annunci_home[] = $pm::load('FAnnuncio',array(['idAnnuncio', '=', $annunci_id[$ran_num[$i]]->getIdAnnuncio()]));
